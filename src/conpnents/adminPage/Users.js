...
const Users = () => {
    ...
    useEffect(() => {
        setFilteredData(userList); // Khởi tạo filteredData với toàn bộ dữ liệu ban đầu
    }, [userList]);
    ...
    const handleSearch = (searchTerm) => {
        if (searchTerm === '') {
            setFilteredData(userList); // Nếu không có từ khóa, hiển thị toàn bộ dữ liệu
        } else {
                ...
                <table className="table table-dark table-hover table-bordered" style={{fontSize: "0.8rem"}}>
                    ...
                    {currentRows?.length > 0 ? (currentRows.map((user, index) => (
                        <tr className="cursor-pointer" key={index}>
                            ...
                            <td>{user.createAt}</td>
                            <td>{user.updateAt === null ? "Chưa cập nhật" : user.updateAt}</td>
                            <td>
