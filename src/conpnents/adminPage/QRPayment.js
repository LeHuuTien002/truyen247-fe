...
import {deleteComic, getAllComics, updateComic} from "../../services/comicService";
...
const QRPayment = () => {
    ...
    useEffect(() => {
        setFilteredData(qrPaymentList); // Khởi tạo filteredData với toàn bộ dữ liệu ban đầu
    }, [qrPaymentList]);
    ...
    const handleSearch = (searchTerm) => {
        if (searchTerm === '') {
            setFilteredData(qrPaymentList); // Nếu không có từ khóa, hiển thị toàn bộ dữ liệu
        } else {
